...
const { status, send } = useWebSocket(`${protocol.replace('http', 'ws')}//${host}/api/notifications/messages`);
    ...
    <div class="justify-start h-full items-center px-8 max-w-7xl mx-auto w-full">
        <PageContainer>
            <div v-if="status === 'OPEN'" class="flex flex-col gap-6 mt-2 w-full">
                ...
                >
                    <span v-if="notification.type !== 'wallet_refund'">{{ notification.message }}</span>
                    <!-- TODO: We can show here some information from payload we expect in message but for now i leave the above -->
                    <span v-else> {{ 'To be discussed what we show here' }}</span>
                    ...
                    <div class="w-full flex justify-end gap-4 absolute bottom-2 right-2">
                        ...
                        <UButton
                            v-if="!notification.readAt && notification.type !== 'wallet_refund'"
                            color="gray"
                            @click="markAsRead(notification.id)"
                            >{{ $t('notifications.markRead') }}</UButton
                        >
                        <UButton
                            v-if="notification.type === 'wallet_refund' && !notification.readAt"
                            color="gray"
                            @click="refund(notification.id)"
                            >{{ $t('notifications.refund') }}</UButton
                        >
                    </div>
                </UCard>
                <UAlert
